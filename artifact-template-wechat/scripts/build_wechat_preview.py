...
async function copyRich() {{
  ...
  copyHost.style.cssText = 'position:fixed;left:-10000px;top:0;width:677px;opacity:.01;pointer-events:none;';
  copyHost.innerHTML = output.outerHTML;
  document.body.appendChild(copyHost);
  const copyOutput = copyHost.firstElementChild;
  const selection = window.getSelection();
  const range = document.createRange(); range.selectNodeContents(copyOutput);
  ...
  try {{
    ...
    for (const item of items) {{
      ...
      verified = (htmlText.match(/<img\\b/gi) || []).length === expected;
    }}
  }} catch (_) {{}}
  status.textContent = verified
    ? `富文本剪贴板校验通过：${{expected}} 张图片；粘贴后请再核对图片数量并保存草稿`
    : `已复制富文本；浏览器未授权回读校验，粘贴后请核对 ${{expected}} 张图片再保存草稿`;
...
ready().catch(() => status.textContent = '图片解码失败，不能复制');
    ...
    # Count the rendered #output fragment, not JavaScript source that contains
    # a literal /<img\b/ clipboard-verification pattern.
    html_images = len(re.findall(r"<img\b", rendered, flags=re.IGNORECASE))
    embedded = len(
        re.findall(
    ...
    if not image_count == html_images == embedded:
        raise RuntimeError(
            "图片数量不一致："
            f"Markdown={image_count}, HTML_img={html_images}, embedded={embedded}"
        )
    return image_count, html_images, embedded, part_count
...
def main() -> None:
    ...
    markdown_images, html_images, embedded, parts = build(
        args.input.resolve(), args.output.resolve()
    )
    print(
        "OK "
        f"markdown_images={markdown_images} "
        f"html_imgs={html_images} "
        f"embedded={embedded} "
        f"parts={parts} "
